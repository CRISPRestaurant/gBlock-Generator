...
def geneToCoord_file(gene_in, table_file):
    ...
    for row in tablereader:
        if row['name'] == gene_in or row['name'] == gene_in.upper():
...
def coordToFasta(regions, fasta_file, outputDir, targetSize, evalAndPrintFunc, indexDir, genome, strand, ext):
    ...
    for region in regions:
        ...
        # print("%s -seq=%s -start=%d -end=%d %s/%s.2bit stdout" % (
            # CONFIG["PATH"]["TWOBITTOFA"], chrom, start - ext, finish + ext, indexDir, genome))
        prog = Popen("%s -seq=%s -start=%d -end=%d %s/%s.2bit stdout" % (
            CONFIG["PATH"]["TWOBITTOFA"], chrom, start - ext, finish + ext, indexDir, genome), stdout=PIPE, shell=True)
...
def main():
    ...
    if args.database:
        ...
        useful_path = CONFIG["PATH"]["GENE_TABLE_INDEX_DIR"] if not ISOFORMS else CONFIG["PATH"]["ISOFORMS_INDEX_DIR"]
